...
export class DetailsPage implements OnInit {
  ...
  constructor(public navCtrl: NavController, public ratesService: RatesService, private storage: Storage, public alertController: AlertController,
  ...
  async ngOnInit() {
    ...
    if(this.sparePartsShopObj.my_review != null || this.sparePartsShopObj.my_review.lenght == 0){
      this.rateData.stars = this.sparePartsShopObj.my_review.stars;
    }
  ...
  async submitRating(rateValue) {
    if (this.rateData.stars === 0) {
      this.sparePartsShopObj.num_Drivix_review++;
    }
    await this.getToken();
    this.rateData.stars = rateValue;
    this.rateData.role_id = this.sparePartsShopObj.id;
  ...
  async getComments() {
    await this.commentsService.getComments(this.sparePartsShopObj.id).then(data => {
      this.comments = [];
  ...
  async addComment() {
    ...
    data['role_id'] = this.sparePartsShopObj.id;
  ...
  openProductsPage(spareShopID) {
    this.router.navigate(['/members/menu/products', spareShopID]);
  }
}
